    ...
    def exists(self, game_id: str = None, phone: str = None) -> bool:
        with open(self.path, mode="r", encoding="utf-8") as arq:    
            for linha in arq:
                ...
                if col[2] == game_id or col[3] == phone:
                    return True
        return False

    def save(self, message_id: int, name: str, game_id: str, phone: str, trophies: int):
        with open(self.path, mode="a", encoding="utf-8") as arq:
            arq.write(",".join([str(message_id), name, game_id, phone, str(trophies)]) + "\n")
